<?php
// FROM HASH: 2071906cd2c2d85d756fccf8de3bf68c
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Đang truy cập');
	...
	<div class="block-container">
		<h2 class="block-tabHeader tabs hScroller" data-xf-init="h-scroller">
			<span class="hScroller-scroll">
				' . '
				<a href="' . $__templater->func('link', array('online', ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == '') ? 'is-active' : '') . '">' . 'Tất cả' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'member', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'member') ? 'is-active' : '') . '">' . 'Thành viên' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'guest', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'guest') ? 'is-active' : '') . '">' . 'Khách' . '</a>
				<a href="' . $__templater->func('link', array('online', null, array('type' => 'robot', ), ), true) . '" class="tabs-tab ' . (($__vars['typeLimit'] == 'robot') ? 'is-active' : '') . '">' . 'Robot' . '</a>
				' . '
			</span>
		</h2>
		';
			...
			foreach ($__vars['activities'] AS $__vars['activity']) {
				...
				if ($__vars['activity']['User']) {
					...
					if ($__vars['activity']['robot_link']) {
						$__finalCompiled .= '<a href="' . $__templater->escape($__vars['activity']['robot_link']) . '" target="_blank" rel="nofollow">' . $__templater->escape($__vars['activity']['robot_title']) . '</a>';
					} else {
					...
					$__finalCompiled .= '
									<h3 class="contentRow-header">' . 'Khách' . '</h3>
								';
				}
				$__finalCompiled .= '
				...
				if ((!$__vars['activity']['User']) OR $__templater->method($__vars['activity']['User'], 'canViewCurrentActivity', array())) {
					...
					if ($__vars['activity']['description']) {
						...
						$__finalCompiled .= '
												<span title="' . $__templater->escape($__vars['activity']['controller_name']) . '::' . $__templater->escape($__vars['activity']['controller_action']) . '">' . 'Đang xem trang không xác định' . '</span>
											';
					} else {
						$__finalCompiled .= '
				...
				if (($__vars['activity']['view_state'] == 'error') AND $__templater->method($__vars['xf']['visitor'], 'canBypassUserPrivacy', array())) {
					$__finalCompiled .= '
						...
						'title' => $__templater->filter('Đang xem lỗi', array(array('for_attr', array()),), false),
					)) . '
											<span class="u-srOnly">' . 'Đang xem lỗi' . '</span>
											</li>
										';
				}
				$__finalCompiled .= '
...
';
	$__templater->modifySidebarHtml('_xfWidgetPositionSidebarf373e16e764a5c11827667920a51a997', $__templater->widgetPosition('online_list_sidebar', array()), 'replace');
