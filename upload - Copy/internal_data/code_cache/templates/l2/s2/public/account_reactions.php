...
return array(
'macros' => array('reactions_list' => array(
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Điểm tương tác đã nhận');
	$__finalCompiled .= '
			...
			if ($__templater->isTraversable($__vars['tabSummary'])) {
				foreach ($__vars['tabSummary'] AS $__vars['reactionId'] => $__vars['total']) {
					...
					if ($__vars['reactionId']) {
						...
						$__finalCompiled .= '
									<bdi>' . 'Tất cả' . '</bdi> ' . $__templater->filter($__vars['total'], array(array('number', array()),array('parens', array()),), true) . '
								';
					}
					$__finalCompiled .= '
							</a>
