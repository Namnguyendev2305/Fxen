...
'thread_list' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__templater->test($__vars['stickyThreads'], 'empty', array()) OR !$__templater->test($__vars['threads'], 'empty', array())) {
		...
		$__finalCompiled .= '
						<div class="structItemContainer-group js-threadList">
							<div class="structItem js-emptyThreadList">
								<div class="structItem-cell">' . 'Không có chủ đề phù hợp với bộ lọc của bạn.' . '</div>
							</div>
							';
		if ($__vars['showDateLimitDisabler']) {
			$__finalCompiled .= '
		...
		if ($__vars['showDateLimitDisabler']) {
			$__finalCompiled .= '
								<div class="structItem js-emptyThreadList">
									<div class="structItem-cell">' . 'Không có chủ đề nào được hiển thị.' . '</div>
								</div>
								' . $__templater->callMacro(null, 'date_limit_disabler', array(
				'forum' => $__vars['forum'],
				'page' => $__vars['page'],
			...
			$__finalCompiled .= '
								<div class="structItem js-emptyThreadList">
									<div class="structItem-cell">' . 'Không có chủ đề trong chuyên mục này' . '</div>
								</div>
							';
		}
		$__finalCompiled .= '
	...
	if ($__vars['filters']['starter_id'] AND $__vars['starterFilter']) {
		$__finalCompiled .= '
			<li><a href="' . $__templater->func('link', array('forums', $__vars['forum'], $__templater->filter($__vars['filters'], array(array('replace', array('starter_id', null, )),), false), ), true) . '"
				class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
				<span class="filterBar-filterToggle-label">' . 'Bắt đầu bởi' . $__vars['xf']['language']['label_separator'] . '</span>
...
'order' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['filters']['order']) {
		...
		if ($__vars['filters']['direction'] == 'asc') {
			$__finalCompiled .= 'Tăng dần';
		} else {
			$__finalCompiled .= 'Giảm dần';
		}
		...
		if ($__vars['xf']['visitor']['user_id']) {
			$__finalCompiled .= '
					<span class="button button--wrap is-disabled">
						' . 'Bạn không đủ quyền để đăng bài ở đây.' . '
