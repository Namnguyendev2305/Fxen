...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Chỉnh sửa trường' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['field']['title'], array(array('htmlspecialchars', array()),), true));
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
...
';
	...
	if ($__vars['hasWrapperTemplate']) {
		$__compilerTemp8 .= '
			...
			'label' => 'Wrapper display HTML',
			'explain' => 'If not empty, this allows you to format the containing markup of this field and its selected values. This offers greater flexibility and customization but you must use the <b>{$value}</b> placeholder for chosen field values to be displayed. Optionally, you may also use <b>{$title}</b> - the field\'s title, <b>{$description}</b> - the field\'s description, and <b>{$fieldId}</b> - the ID of this field (<span id="FieldId">' . $__templater->escape($__vars['field']['field_id']) . '</span>).',
		)) . '
			';
	}
