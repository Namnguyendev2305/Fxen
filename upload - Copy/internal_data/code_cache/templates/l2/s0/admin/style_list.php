<?php
// FROM HASH: 6cc0a62664cd1ae8c21cab7db16a120b
return array(
'macros' => array('style_list' => array(
'arguments' => function($__templater, array $__vars) { return array(
		'children' => '!',
		'defaultStyleId' => '!',
		'depth' => '1',
		'canSupportStyleArchives' => false,
	); },
	...
	if (!$__templater->test($__vars['children'], 'empty', array())) {
		...
		if ($__templater->isTraversable($__vars['children'])) {
			...
			' . $__templater->callMacro(null, 'style_list_entry', array(
					...
					'depth' => $__vars['depth'],
					'canSupportStyleArchives' => $__vars['canSupportStyleArchives'],
				), $__vars) . '
		';
			}
		}
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['style']['style_id']) {
		$__compilerTemp2[] = array(
			...
			'submit' => 'true',
			'tooltip' => 'Enable / disable user selection \'' . $__vars['style']['title'] . '\'',
			'_type' => 'toggle',
			'html' => '',
		);
		$__compilerTemp2[] = array(
	...
	if ($__vars['style']['style_id']) {
		...
		if ($__vars['canSupportStyleArchives']) {
			$__compilerTemp3 .= '
							<a href="' . $__templater->func('link', array('styles/export-archive', $__vars['style'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Export as archive' . '</a>
						';
		}
		$__compilerTemp2[] = array(
			'label' => '&#8226;&#8226;&#8226;',
			...
			'html' => '
				<div class="menu" data-menu="menu" aria-hidden="true" data-menu-builder="dataList">
					<div class="menu-content">
						...
						<a href="' . $__templater->func('link', array('styles/export', $__vars['style'], ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Export as XML' . '</a>
						' . $__compilerTemp3 . '
						<a href="' . $__templater->func('link', array('styles/customized-components', $__vars['style'], ), true) . '" class="menu-linkRow">' . 'Tùy chỉnh các thành phần' . '</a>
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Các giao diện');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Thêm giao diện', array(
