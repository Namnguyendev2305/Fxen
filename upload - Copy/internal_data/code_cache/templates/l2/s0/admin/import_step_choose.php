...
<div class="js-importConfigForm">
	...
	if ($__vars['isCoreImporter'] AND $__vars['canRetainIds']) {
		...
		<div class="blockMessage blockMessage--warning blockMessage--iconic">
			' . 'In order to avoid unexpected user merges, please ensure your <a href="' . $__templater->func('link', array('users/edit', $__vars['xf']['visitor'], ), true) . '" target="_blank">current user</a> is using a username and email that is not used by any users in the source database!' . '
		</div>
	';
	}
	$__finalCompiled .= '
	...
	$__finalCompiled .= $__templater->form('
		...
		'explain' => '
						' . 'Some steps may require other steps as a prerequisite. Unselected steps will be automatically included if a selected step requires them.' . '
					',
		'hint' => '
						<br />
