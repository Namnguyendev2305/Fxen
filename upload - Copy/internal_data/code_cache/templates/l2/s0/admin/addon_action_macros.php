	...
	if (!$__vars['errors']) {
		...
		if ($__vars['warnings']) {
			...
			<div class="block-body">
				...
				'label' => 'Tôi đã xem lại các cảnh cáo và tôi vẫn muốn tiến hành',
				'_type' => 'option',
			)), array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['errors']) {
		...
		<div class="block-rowMessage block-rowMessage--error block-rowMessage--iconic">
			' . 'Các lỗi sau phải được giải quyết trước khi tiếp tục' . $__vars['xf']['language']['label_separator'] . '
			<ul>
				';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['warnings']) {
		...
		<div class="block-rowMessage block-rowMessage--warning block-rowMessage--iconic">
			' . 'Để tránh những vấn đề tiềm ẩn, hãy giải quyết các cảnh báo sau đây trước khi tiếp tục' . $__vars['xf']['language']['label_separator'] . '
			<ul>
				';
