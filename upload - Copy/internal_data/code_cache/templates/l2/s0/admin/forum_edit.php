...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['forum'], 'isInsert', array())) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Thêm diễn đàn');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
...
';
	...
	if (!$__templater->test($__vars['availableFields'], 'empty', array())) {
		...
		$__compilerTemp3 .= '
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . '
					<a href="' . $__templater->func('link', array('custom-thread-fields/add', ), true) . '" target="_blank">' . 'Thêm trường' . '</a>
				', array(
			'label' => 'Các trường có sẵn',
		)) . '
			';
	...
	if (!$__templater->test($__vars['prefixesGrouped'], 'empty', array())) {
			...
			foreach ($__vars['prefixGroups'] AS $__vars['prefixGroupId'] => $__vars['prefixGroup']) {
				if ($__vars['prefixesGrouped'][$__vars['prefixGroupId']]) {
					$__compilerTemp6[] = array(
						...
						'label' => ($__vars['prefixGroupId'] ? $__vars['prefixGroup']['title'] : 'Không nhóm'),
						'_type' => 'optgroup',
						'options' => array(),
					);
		...
		$__compilerTemp5 .= $__templater->formSelectRow(array(
			...
			'label' => 'Tiền tố chủ đề mặc định',
			'explain' => 'Bạn có thể chỉ định một tiền tố của chủ đề được tự động chọn khi khách truy cập tạo chủ đề mới trong diễn đàn này. Tiền tố được chọn <b>phải</b> được chọn trong danh sách \'các tiền tố có sẵn\' ở trên.',
		)) . '
		...
		$__compilerTemp5 .= '
				' . $__templater->formRow('
					' . $__templater->filter('Không có', array(array('parens', array()),), true) . ' <a href="' . $__templater->func('link', array('thread-prefixes/add', ), true) . '" target="_blank">' . 'Thêm tiền tố' . '</a>
				', array(
			'label' => 'Tiền tố có sẵn',
		)) . '
	...
	array(
		...
		'hint' => 'Nếu bị vô hiệu hóa, các bài viết được đăng (trực tiếp) trong diễn đàn này sẽ không đóng góp vào tổng số bài viết của người đăng. Ngoài ra, nếu diễn đàn này hỗ trợ các chủ đề câu hỏi, các giải pháp sẽ không đóng góp vào tổng số giải pháp của người dùng.',
		'_type' => 'option',
	),
	array(
		'name' => 'find_new',
		'selected' => $__vars['forum']['find_new'],
		'label' => 'Bao gồm các chủ đề từ diễn đàn này khi người dùng nhấp vào "Bài viết mới"',
