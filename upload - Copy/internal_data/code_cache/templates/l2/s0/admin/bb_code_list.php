...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Tùy chỉnh BB Codes');
	$__finalCompiled .= '
...
';
	...
	<div class="buttonGroup">
		' . $__templater->button('Thêm BB code', array(
			...
			foreach ($__vars['bbCodes'] AS $__vars['bbCode']) {
				...
				if (!$__vars['exportView']) {
					$__compilerTemp3[] = array(
						...
						'submit' => 'true',
						'tooltip' => 'Bật / Tắt \'' . (('[' . $__vars['bbCode']['bb_code_id']) . ']') . '\'',
						'_type' => 'toggle',
						'html' => '',
					);
					$__compilerTemp3[] = array(
