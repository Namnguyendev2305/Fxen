...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Bản mẫu đã lỗi thời');
	$__finalCompiled .= '
';
...
';
	...
	<div class="blockMessage blockMessage--success blockMessage--iconic">
		' . 'Tất cả các mẫu tự động có thể hợp nhất đã được xử lý. Bất kỳ mẫu còn lại nào cũng cần phải được xử lý thủ công.' . '
	</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	if ($__vars['total']) {
		$__compilerTemp1 .= '
		' . $__templater->button('Tự động hợp nhất', array(
	...
	' . $__templater->button('Tùy chỉnh các thành phần', array(
					...
					if ($__templater->isTraversable($__vars['outdatedForStyle'])) {
						foreach ($__vars['outdatedForStyle'] AS $__vars['outdated']) {
							...
							if ($__vars['outdated']['outdated_by_date']) {
								$__compilerTemp4 .= '
													<li>' . 'Sửa lần cuối' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['outdated']['template']['last_edit_date'], array(
								))) . '</li>
								...
								$__compilerTemp4 .= '
													<li>' . 'Phiên bản tùy chỉnh' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['outdated']['template']['version_string']) . '</li>
													<li>' . 'Phiên bản gốc' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['outdated']['parent_version_string']) . '</li>
												';
							}
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Không có mẫu lỗi thời.' . '</div>
';
	}
