<?php
// FROM HASH: f1d03390348b12d94c10b77fd84b45d2
return array(
...
';
	...
	if ($__templater->isTraversable($__vars['groups'])) {
		foreach ($__vars['groups'] AS $__vars['_group']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->func('link', array('options/groups/', $__vars['_group'], ), true) . '" class="menu-linkRow ' . (($__vars['_group']['group_id'] == $__vars['group']['group_id']) ? 'is-selected' : '') . ' ' . ($__vars['_group']['advanced'] ? 'acp--advanced' : '') . '">
						';
...
';
	...
	if ($__vars['canAdd']) {
		$__compilerTemp2 .= '
		' . $__templater->button('Thêm tùy chọn', array(
	...
	if ($__templater->method($__vars['group'], 'canEdit', array())) {
		$__compilerTemp3 .= '
		' . $__templater->button('Chỉnh sửa nhóm', array(
	...
	$__templater->pageParams['pageAction'] = $__templater->preEscaped($__templater->func('trim', array('
	' . $__compilerTemp2 . '
	' . $__compilerTemp3 . '
'), false));
	$__finalCompiled .= '
