...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Các danh hiệu');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Thêm danh hiệu', array(
		...
		if ($__templater->isTraversable($__vars['trophies'])) {
			foreach ($__vars['trophies'] AS $__vars['trophy']) {
				$__compilerTemp1 .= '
					...
					'hint' => 'Điểm' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['trophy']['trophy_points']),
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Không có danh hiệu nào được tạo.' . '</div>
';
	}
	$__finalCompiled .= '
