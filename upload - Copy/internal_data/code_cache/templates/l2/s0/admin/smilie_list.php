...
';
	...
	<div class="buttonGroup">
		' . $__templater->button('Thêm mặt cười', array(
		...
		'title' => 'Thêm tùy chọn',
			...
			<div class="menu" data-menu="menu" aria-hidden="true">
				<div class="menu-content">
					...
					<a href="' . $__templater->func('link', array('smilies/sort', ), true) . '" class="menu-linkRow" data-xf-click="overlay">' . 'Sắp xếp' . '</a>
					<a href="' . $__templater->func('link', array('smilies/import', ), true) . '" class="menu-linkRow">' . 'Nhập' . '</a>
					<a href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '" class="menu-linkRow">' . 'Xuất' . '</a>
				</div>
			</div>
		</div>
	</div>
...
';
	...
	if ($__templater->isTraversable($__vars['smilieData']['smilieCategories'])) {
		foreach ($__vars['smilieData']['smilieCategories'] AS $__vars['smilieCategoryId'] => $__vars['smilieCategory']) {
			...
			if ($__compilerTemp3) {
				...
				if (($__templater->func('count', array($__vars['smilieData']['smilieCategories'], ), false) > 1)) {
					$__compilerTemp6 .= '
									' . 'Chưa có mặt cười nào được thêm vào thể loại này.' . '
								';
				} else {
					$__compilerTemp6 .= '
	...
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" role="tablist">
			...
			<a class="tabs-tab ' . ($__vars['exportView'] ? 'is-active' : '') . '" role="tab" tabindex="0" aria-controls="export-mode" href="' . $__templater->func('link', array('smilies', array(), array('export' => 1, ), ), true) . '">' . 'Xuất' . '</a>
			' . '
		</h2>
