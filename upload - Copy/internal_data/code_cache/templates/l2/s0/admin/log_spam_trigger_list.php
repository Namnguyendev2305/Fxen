		...
		if ($__templater->isTraversable($__vars['entries'])) {
			foreach ($__vars['entries'] AS $__vars['entry']) {
				$__compilerTemp1 .= '
					...
					'label' => ($__vars['entry']['User'] ? $__templater->escape($__vars['entry']['User']['username']) : 'Tài khoản không xác định') . ' - ' . $__templater->escape($__vars['entry']['details']),
					'href' => $__templater->func('link', array('logs/spam-trigger', $__vars['entry'], ), false),
					'overlay' => 'true',
					'explain' => '
								<ul class="listInline listInline--bullet">
									<li>' . (($__vars['entry']['result'] == 'moderated') ? 'Cần kiểm duyệt' : 'Đã từ chối') . '</li>
