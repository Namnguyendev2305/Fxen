...
';
	...
	<div class="blockMessage blockMessage--error blockMessage--iconic">
		' . 'Hỗ trợ Unicode đầy đủ đã được kích hoạt trong config.php nhưng cơ sở dữ liệu của bạn không được thiết lập để hỗ trợ này. Hỗ trợ Unicode đầy đủ nên được vô hiệu hóa hoặc lỗi có thể xảy ra.' . '
	</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	<div class="blockMessage blockMessage--important blockMessage--iconic">
		<a href="' . $__templater->func('link', array('templates/outdated', ), true) . '"> ' . 'Có những mẫu có thể đã lỗi thời. Nhấn vào đây để xem lại.' . '</a>
	</div>
';
	}
	$__finalCompiled .= '
...
';
	if ($__templater->method($__vars['xf']['visitor'], 'hasAdminPermission', array('viewLogs', )) AND $__vars['serverErrorLogs']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--error blockMessage--iconic">
		<a href="' . $__templater->func('link', array('logs/server-errors', ), true) . '"> ' . 'Lỗi máy chủ đã được ghi lại. Bạn nên xem lại những điều này.' . '</a>
	</div>
';
	}
	$__finalCompiled .= '
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--important blockMessage--iconic">
		' . 'Việc xây dựng lại đang chờ hoàn thành. <a href="' . $__templater->func('link', array('tools/run-job', ), true) . '">Tiếp tục chạy chúng.</a>' . '
	</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	<div class="blockMessage blockMessage--error blockMessage--iconic">
		<a href="' . $__templater->func('link', array('tools/file-check/results', $__vars['firstFileCheck'], ), true) . '">
			' . 'Có ' . $__templater->filter(($__vars['firstFileCheck']['total_missing'] + $__vars['firstFileCheck']['total_inconsistent']), array(array('number', array()),), true) . ' tệp bị thiếu hoặc có nội dung không mong muốn. Bạn nên xem lại những điều này.' . '
		</a>
	</div>
';
	}
...
';
	...
	<div class="block">
		<div class="block-container">
			...
			'name' => 'query',
			'placeholder' => 'Tên thành viên, địa chỉ email, IP' . $__vars['xf']['language']['ellipsis'],
			'value' => '',
		), array(
...
';
	...
	<div class="block">
		<div class="block-container">
			<h2 class="block-header"><a href="' . $__templater->func('link', array('stats', ), true) . '">' . 'Số liệu thống kê' . '</a></h2>
...
' . '
		...
		$__finalCompiled .= '
					...
					<dl class="pairs pairs--columns">
						<dt>' . 'cURL SSL version' . '</dt>
						<dd>' . ($__templater->escape($__vars['envReport']['ssl_version']) ?: 'N/A') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . 'OpenSSL version' . '</dt>
						<dd>' . ($__templater->escape($__vars['envReport']['openssl_version']) ?: 'N/A') . '
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . 'Suhosin enabled' . '</dt>
						<dd>' . ($__vars['envReport']['suhosin'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . 'Imagick support' . '</dt>
						<dd>' . ($__vars['envReport']['imagick'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . 'EXIF support' . '</dt>
						<dd>' . ($__vars['envReport']['exif'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . '<code>GZip</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['gzip'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . '<code>mbstring</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['mbstring'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . '<code>gmp</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['gmp'] ? 'Có' : 'Không') . '</dd>
					</dl>
					<dl class="pairs pairs--columns">
						<dt>' . '<code>ZipArchive</code> support' . '</dt>
						<dd>' . ($__vars['envReport']['zip'] ? 'Có' : 'Không') . '</dd>
					</dl>
				</div>
			</div>
		</div>
