...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (strlen(trim($__compilerTemp1)) > 0) {
		...
		<div class="node-subNodesFlat">
			<span class="node-subNodesLabel">' . 'Chuyên mục con' . $__vars['xf']['language']['label_separator'] . '</span>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (($__vars['xf']['options']['forumsDefaultPage'] != 'forums') OR (($__vars['xf']['options']['indexRoute'] AND ($__vars['xf']['options']['indexRoute'] != 'forums/')))) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Danh sách diễn đàn');
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
