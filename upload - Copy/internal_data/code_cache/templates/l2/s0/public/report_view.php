...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Nội dung được báo cáo' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['report']['title']));
	$__finalCompiled .= '
';
...
';
	$__templater->breadcrumb($__templater->preEscaped('Báo cáo'), $__templater->func('link', array('reports', ), false), array(
	));
	$__finalCompiled .= '
';
	...
	if ($__vars['report']['link']) {
		$__finalCompiled .= '
			<div class="block-footer">
				<span class="block-footer-controls">' . $__templater->button('Đi đến bài viết', array(
...
<div class="block">
	<div class="block-container">
		<h3 class="block-header">' . 'Bình luận' . '</h3>
	...
	if ($__templater->isTraversable($__vars['comments'])) {
		foreach ($__vars['comments'] AS $__vars['comment']) {
			...
			if ($__vars['comment']['is_report']) {
				$__finalCompiled .= '
											<li><span class="label label--accent">' . 'Báo cáo' . '</span></li>
										';
			}
			$__finalCompiled .= '
									</ul>
			...
			if ($__vars['comment']['state_change']) {
				$__finalCompiled .= '
									<div class="message-minorHighlight">' . 'Tình trạng đã thay đổi' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__templater->method($__vars['report'], 'getReportState', array($__vars['comment']['state_change'], ))) . '</div>
								';
			}
			$__finalCompiled .= '
...
<div class="block">
	<div class="block-container">
		...
		<h2 class="block-tabHeader tabs" data-xf-init="tabs" role="tablist">
			<a class="tabs-tab is-active" role="tab" tabindex="0" aria-controls="' . $__templater->func('unique_id', array('reportUpdate', ), true) . '">' . 'Cập nhật' . '</a>
			<a class="tabs-tab" role="tab" tabindex="0" aria-controls="' . $__templater->func('unique_id', array('reportReassign', ), true) . '">' . 'Chỉ định' . '</a>
		</h2>
	...
	if ($__vars['assignedToOther']) {
		$__compilerTemp2 .= '
							' . $__templater->formRow('
								' . 'Báo cáo này hiện được giao cho ' . $__templater->escape($__vars['report']['AssignedUser']['username']) . '.' . '
								<div>
									' . $__templater->button('
										' . 'Xử lý báo cáo' . '
									', array(
	...
	if ($__templater->method($__vars['report'], 'isClosed', array())) {
		$__compilerTemp3 .= 'Bình luận của bạn sẽ mở lại báo cáo này.';
	}
	$__compilerTemp4 = '';
	if (!$__templater->method($__vars['report'], 'isClosed', array())) {
		$__compilerTemp4 .= '
		...
		array(
			'value' => 'assigned',
			'label' => (($__vars['report']['report_state'] == 'assigned') ? 'Bỏ giao' : 'Giao'),
			'_type' => 'option',
		),
		array(
			'value' => 'resolved',
		...
		array(
			...
			'name' => 'send_alert',
			'label' => 'Gửi đề nghị/ từ chối cảnh cáo' . $__vars['xf']['language']['label_separator'],
			'_dependent' => array('
			...
			'explain' => 'Bất kỳ đề nghị/từ chối sẽ được tự động thêm vào nhận xét hiện tại.',
		)) . '
							';
	}
	$__finalCompiled .= $__templater->form('
		...
		'label' => 'Nội dung',
		'explain' => '
									' . 'Phản hồi của bạn sẽ chỉ được hiển thị cho người điều hành' . '
									' . $__compilerTemp3 . '
								',
	)) . '
