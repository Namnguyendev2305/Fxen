<?php
// FROM HASH: 81ad91a343eea674200e3fe0e5b46948
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['uneditableTags']) {
		...
		', array(
			'label' => 'Từ khóa không thể chỉnh sửa',
		)) . '
	';
	}
	$__finalCompiled .= '
	...
	if ($__vars['editLink']) {
		$__finalCompiled .= '
				<a href="' . $__templater->escape($__vars['editLink']) . '" class="u-concealed--icon"
					...
					data-xf-init="tooltip" title="' . $__templater->filter('Sửa từ khóa', array(array('for_attr', array()),), true) . '">' . $__templater->escape($__vars['tagIcon']) . '</a>
			';
	} else {
		$__finalCompiled .= '
