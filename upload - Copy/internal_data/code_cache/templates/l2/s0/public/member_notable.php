		...
		if ($__vars['showFooter']) {
			$__finalCompiled .= '
				<div class="memberOverviewBlock-seeMore">
					<a href="' . $__templater->func('link', array('members', null, array('key' => $__vars['memberStat']['member_stat_key'], ), ), true) . '">' . 'Xem thêm' . $__vars['xf']['language']['ellipsis'] . '</a>
				</div>
			';
		}
		$__finalCompiled .= '
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped(($__vars['active'] ? $__templater->escape($__vars['active']['title']) : 'Thành viên tiêu biểu'));
	$__finalCompiled .= '
...
';
	if ($__vars['userNotFound']) {
		$__finalCompiled .= '
	<div class="blockMessage blockMessage--error blockMessage--iconic">' . 'Thành viên bạn nhập không tìm thấy. Xin hãy nhập tên đầy đủ của thành viên.' . '</div>
';
	}
	$__finalCompiled .= '
...
';
	...
	<section class="block">
		...
		if ($__vars['active']) {
			...
			if ($__compilerTemp2) {
				$__finalCompiled .= '
						<li class="block-row">' . 'Không có thành viên nào khớp với tiêu chí được chỉ định.' . '</li>
					';
			}
			$__finalCompiled .= '
				</ol>
