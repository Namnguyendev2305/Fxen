<?php
// FROM HASH: 689c1b34a5c4b93abcfd27c4d60c294b
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['thread']['reply_count']) {
		$__compilerTemp1 .= '
			...
			'class' => 'u-spaceAfter',
		)) . 'Trả lời' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '</a>
						';
	}
	$__compilerTemp1 .= '
						';
...
'extensions' => array('status_new' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['thread'], 'isUnread', array())) {
		$__finalCompiled .= '
								<li><span class="message-newIndicator" title="' . ((!$__templater->method($__vars['post'], 'isUnread', array())) ? 'New replies' : '') . '">' . 'Mới' . '</span></li>
							';
	}
	$__finalCompiled .= '
						';
...
'status_state' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if (!$__vars['thread']['discussion_open']) {
		$__finalCompiled .= '
								<li>
									<i class="structItem-status structItem-status--locked" aria-hidden="true" title="' . $__templater->filter('Đã khóa', array(array('for_attr', array()),), true) . '"></i>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
					...
					<div class="message-content js-messageContent" data-xf-init="lightbox">
						<div class="message-userContent lbContainer js-lbContainer"
							...
							<article class="message-body">
								' . $__templater->func('snippet', array($__vars['post']['message'], 750, array('stripQuote' => true, 'bbWrapper' => true, ), ), true) . '
							</article>
						</div>
					</div>
				</div>
				<div class="articlePreview-links">
					...
					<a href="' . $__templater->escape($__vars['link']) . '">' . 'Xem bài viết' . ' &raquo;</a>
				</div>
			</div>
		</div>
		...
		'defaultname' => $__vars['thread']['username'],
				...
				<li class="articlePreview-by">' . 'Bởi ' . $__templater->escape($__vars['username']) . '' . '</li>
	...
	if ($__vars['thread']['reply_count']) {
		$__finalCompiled .= '
					<li class="articlePreview-replies"><a href="' . $__templater->escape($__vars['linkUnread']) . '"
						title="' . 'Trả lời' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '">
						' . $__templater->fontAwesome('fa-comment', array(
			'class' => 'u-spaceAfter',
		)) . '<span class="articlePreview-repliesLabel">' . 'Trả lời' . $__vars['xf']['language']['label_separator'] . ' </span>' . $__templater->filter($__vars['thread']['reply_count'], array(array('number', array()),), true) . '</a></li>
				';
	}
	$__finalCompiled .= '
				';
