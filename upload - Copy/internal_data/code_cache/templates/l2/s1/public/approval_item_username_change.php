				...
				if ($__vars['change']['change_reason']) {
					$__compilerTemp3 .= '
							<a role="button"
									...
									title="' . 'Change reason' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['change']['change_reason']) . '">' . $__templater->func('trim', array('
					...
					if ($__vars['change']['reject_reason']) {
						$__compilerTemp4 .= '
								<a role="button"
									...
									data-xf-init="tooltip"
									data-trigger="hover focus click"
									title="' . 'Lý do' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['change']['reject_reason']) . '">' . $__templater->func('trim', array('
									' . $__templater->fontAwesome('fa-info-circle', array(
						)) . '
								'), false) . '</a>
							';
...
';
	...
	array(
		...
		'html' => '
				<div class="formRow-explain">' . 'Sẽ được hiển thị cho thành viên nếu được cung cấp.' . '</div>
			',
		'_type' => 'option',
	))) . '
	...
	' . $__templater->formCheckBox(array(
		...
		'checked' => true,
		'label' => '
			' . 'Thông báo đến thành viên khi hành động đến' . '
		',
		'_type' => 'option',
	))) . '
