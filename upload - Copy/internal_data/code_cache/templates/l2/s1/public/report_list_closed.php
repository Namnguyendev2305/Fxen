...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Báo cáo đã đóng: ' . $__templater->func('date', array($__vars['timeFrame']['0'], ), true) . ' - ' . $__templater->func('date', array($__vars['timeFrame']['1'], ), true) . '');
	$__finalCompiled .= '
...
<div class="block">
	...
	<div class="block-container">
		<h2 class="block-tabHeader tabs">
			<a href="' . $__templater->func('link', array('reports', ), true) . '" class="tabs-tab">' . 'Báo cáo chưa duyệt' . '</a>
			<a href="' . $__templater->func('link', array('reports/closed', ), true) . '" class="tabs-tab is-active">' . 'Báo cáo đã đóng' . '</a>
		</h2>
	...
	if ($__vars['page'] > 1) {
		$__finalCompiled .= '
					' . $__templater->button('Mới hơn' . ' &rarr;', array(
			'href' => $__templater->func('link', array('reports/closed', null, array('page' => $__vars['page'] - 1, ), ), false),
		), '', array(
		)) . '
				';
