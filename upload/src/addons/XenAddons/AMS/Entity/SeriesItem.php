 ...
 * @property \XF\Mvc\Entity\AbstractCollection|\XenAddons\AMS\Entity\SeriesWatch[] $Watch
...
class SeriesItem extends Entity implements \XF\BbCode\RenderableContentInterface, \XF\Entity\LinkableInterface
{
	...
	public function isSearchEngineIndexable()
	{
		...
		if ($this->article_count < 1) // TODO make this an option
		{
			return false;
		}
	...
	public function getBreadcrumbs($includeSelf = true)
	{
		$breadcrumbs[] = [
			...
			'value' => \XF::phrase('xa_ams_series_list')
		];
	...
	public function getLdStructuredData(int $page = 1, array $extraData = []): array
	{
		...
		$output = [
			...
			'@id'                 => $router->buildLink('canonical:ams/series', $this),
			'name'                => $this->title,
	...
	{
		$this->article_count = $this->db()->fetchOne("
			SELECT COUNT(*)
				FROM xf_xa_ams_series_part
				WHERE series_id = ?
		", $this->series_id);
	...
	protected function seriesHidden($hardDelete = false)
	{
		...
		$alertRepo->fastDeleteAlertsForContent('ams_series_part', $this->series_part_ids); 
	}	
	...
	{
		$route = ($canonical ? 'canonical:' : '') . 'ams/series';
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'article_count' => ['type' => self::UINT, 'default' => 0],
