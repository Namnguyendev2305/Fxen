...
class Comment extends Entity implements QuotableInterface, \XF\BbCode\RenderableContentInterface
{
	...
	public function canChangeDate(&$error = null)
	{
		...
		return (
			$visitor->user_id
			&& $this->Content->hasPermission('changeDateComment')
		);
	}
	...
	protected function adjustUserCommentCountIfNeeded($amount, $userId = null)
	{
		...
		if ($userId)
		{
			$this->db()->query("
				UPDATE xf_user
				SET xa_ams_comment_count = GREATEST(0, xa_ams_comment_count + ?)
				WHERE user_id = ?
			", [$amount, $userId]);
		}
	}
	...
	protected function _postDelete()
	{
		...
		$db->delete('xf_approval_queue', 'content_id = ? AND content_type = ?', [$this->comment_id, 'ams_comment']);
		$db->delete('xf_deletion_log', 'content_id = ? AND content_type = ?', [$this->comment_id, 'ams_comment']);
		$db->delete('xf_edit_history', 'content_id = ? AND content_type = ?', [$this->comment_id, 'ams_comment']);
