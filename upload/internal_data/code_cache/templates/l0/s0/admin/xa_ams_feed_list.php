<?php
// FROM HASH: 88b9352759c4a6abaa0b2f8882a360e3
return array(
...
';
	...
	' . $__templater->button('Add feed', array(
		'href' => $__templater->func('link', array('xa-ams/feeds/add', ), false),
		'icon' => 'add',
	), '', array(
	)) . '
');
...
';
		...
		if ($__templater->isTraversable($__vars['feeds'])) {
			foreach ($__vars['feeds'] AS $__vars['feed']) {
				$__compilerTemp1 .= '
					...
					'href' => $__templater->func('link', array('xa-ams/feeds/edit', $__vars['feed'], ), false),
				...
				array(
					'href' => $__templater->func('link', array('xa-ams/feeds/import', $__vars['feed'], ), false),
					'_type' => 'action',
					'html' => 'Import now',
				),
				array(
	...
	', array(
			'action' => $__templater->func('link', array('xa-ams/feeds/toggle', ), false),
			'class' => 'block',
			'ajax' => 'true',
		)) . '
';
