...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Thống kê thành viên');
	$__finalCompiled .= '
';
...
';
	...
	' . $__templater->button('Thêm thống kê thành viên', array(
		...
		if ($__templater->isTraversable($__vars['memberStats'])) {
			foreach ($__vars['memberStats'] AS $__vars['memberStat']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Bật / Tắt \'' . $__vars['memberStat']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				))) . '
					';
