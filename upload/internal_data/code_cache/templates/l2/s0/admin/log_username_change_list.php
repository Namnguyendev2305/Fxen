...
';
			...
			if ($__vars['entry']['change_reason']) {
				$__compilerTemp3 .= '
								<a role="button"
									...
									title="' . 'Change reason' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['entry']['change_reason']) . '">' . $__templater->func('trim', array('
				...
				if ($__vars['entry']['reject_reason']) {
					$__compilerTemp4 .= '
									<a role="button"
										...
										data-xf-init="tooltip"
										data-trigger="hover focus click"
										title="' . 'Lý do' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['entry']['reject_reason']) . '">' . $__templater->func('trim', array('
										' . $__templater->fontAwesome('fa-info-circle', array(
					)) . '
									'), false) . '</a>
								';
	...
	if ($__compilerTemp2) {
		$__compilerTemp1 .= '
			...
			'colspan' => '7',
			'_type' => 'cell',
			'html' => 'Không có nhật ký nào được ghi nhận.',
		))) . '
				';
	}
