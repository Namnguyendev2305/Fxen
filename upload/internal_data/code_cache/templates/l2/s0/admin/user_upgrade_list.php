...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Nâng cấp thành viên');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Thêm nâng cấp thành viên', array(
...
';
		...
		if ($__templater->isTraversable($__vars['upgrades'])) {
			foreach ($__vars['upgrades'] AS $__vars['upgrade']) {
				$__compilerTemp1 .= '
					...
					'submit' => 'true',
					'tooltip' => 'Bật / Tắt \'' . $__vars['upgrade']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				),
				array(
					...
					'html' => '
								<div class="menu" data-menu="menu" aria-hidden="true">
									<div class="menu-content">
										...
										<a href="' . $__templater->func('link', array('user-upgrades/active', $__vars['upgrade'], ), true) . '" class="menu-linkRow">' . 'Xem thành viên' . '</a>
										<a href="' . $__templater->func('link', array('user-upgrades/expired', $__vars['upgrade'], ), true) . '" class="menu-linkRow">' . 'Xem nâng cấp hết hạn' . '</a>
										<a href="' . $__templater->func('link', array('user-upgrades/manual', $__vars['upgrade'], ), true) . '" class="menu-linkRow">' . 'Nâng cấp thành viên thủ công' . '</a>
									</div>
								</div>
							',
				))) . '
	...
	<div class="block">
		<div class="block-container">
			<h2 class="block-header"><a href="' . $__templater->func('link', array('user-upgrades/active', ), true) . '">' . 'Nâng cấp thành viên đang hoạt động' . '</a></h2>
			';
		if (!$__templater->test($__vars['activeUpgrades'], 'empty', array())) {
			$__finalCompiled .= '
				...
				<div class="block-footer block-footer--split">
					...
					<span class="block-footer-controls">' . $__templater->button('Xem thêm' . $__vars['xf']['language']['ellipsis'], array(
	...
	<div class="block">
		<div class="block-container">
			<h2 class="block-header"><a href="' . $__templater->func('link', array('user-upgrades/expired', ), true) . '">' . 'Gói nâng cấp đã hết hạn' . '</a></h2>
			';
