<?php
// FROM HASH: 144ada3af342dbdb2d54c07962a9ead1
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__templater->method($__vars['addOn'], 'canUpgrade', array())) {
		...
		$__finalCompiled .= '
			';
		if ($__vars['addOnDirWritable'] AND $__templater->method($__vars['addOn'], 'canDeleteFiles', array())) {
			$__finalCompiled .= '
				' . $__templater->button('
					' . 'Delete files' . '
				', array(
				'href' => $__templater->func('link', array('add-ons/delete-files', $__vars['addOn'], ), false),
				'overlay' => 'true',
				'data-cache' => '0',
			), '', array(
			)) . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['addOn']['dev'] OR $__vars['addOn']['dev_url']) {
		...
		if ($__vars['addOn']['dev'] AND $__vars['addOn']['dev_url']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Nhà phát triển' . $__vars['xf']['language']['label_separator'] . '</span> <a href="' . $__templater->escape($__vars['addOn']['dev_url']) . '" target="_blank">' . $__templater->escape($__vars['addOn']['dev']) . '</a>
					';
		} else if ($__vars['addOn']['dev']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Nhà phát triển' . $__vars['xf']['language']['label_separator'] . '</span> ' . $__templater->escape($__vars['addOn']['dev']) . '
					';
		} else if ($__vars['addOn']['dev_url']) {
			$__finalCompiled .= '
						<span class="addOnList-row-noSearch">' . 'Nhà phát triển' . $__vars['xf']['language']['label_separator'] . '</span> <a href="' . $__templater->escape($__vars['addOn']['dev_url']) . '" target="_blank">' . $__templater->escape($__vars['addOn']['dev_url']) . '</a>
					';
		}
		$__finalCompiled .= '
				</li>
