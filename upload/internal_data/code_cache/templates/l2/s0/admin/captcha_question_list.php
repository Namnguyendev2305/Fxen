...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Hỏi và trả lời CAPTCHAs');
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('Thêm câu hỏi', array(
		...
		if ($__templater->isTraversable($__vars['questions'])) {
			foreach ($__vars['questions'] AS $__vars['question']) {
				...
				array(
					...
					'submit' => 'true',
					'tooltip' => 'Bật / Tắt \'' . $__vars['question']['question'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				),
				array(
