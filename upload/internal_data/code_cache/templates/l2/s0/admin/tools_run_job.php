...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Đang thực hiện' . $__vars['xf']['language']['ellipsis']);
	$__finalCompiled .= '
';
...
<form action="' . $__templater->func('link', array('tools/run-job', ), true) . '" method="post" class="blockMessage" data-xf-init="auto-submit">
	...
	if ($__vars['status']) {
		...
		$__finalCompiled .= 'Đang thực hiện' . $__vars['xf']['language']['ellipsis'];
	}
	$__finalCompiled .= '</div>
