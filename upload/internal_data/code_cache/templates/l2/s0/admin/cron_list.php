...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Mục Cron');
	$__finalCompiled .= '
			...
			foreach ($__vars['entries'] AS $__vars['cron']) {
				...
				if ($__vars['cron']['active']) {
					...
					if (((!$__vars['cron']['addon_id']) OR $__vars['cron']['AddOn']['active'])) {
						$__compilerTemp2 .= '
											' . (('Chạy tiếp theo' . ': ') . $__templater->func('date_time', array($__vars['cron']['next_run'], ), true)) . '
										';
					} else {
						$__compilerTemp2 .= '
...
,array(
					...
					'submit' => 'true',
					'tooltip' => 'Bật / Tắt \'' . $__vars['cron']['title'] . '\'',
					'_type' => 'toggle',
					'html' => '',
				));
				if (((!$__vars['cron']['addon_id']) OR $__vars['cron']['AddOn']['active'])) {
