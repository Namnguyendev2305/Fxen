...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['allowInlineMod'] AND $__templater->method($__vars['article'], 'canUseInlineModeration', array())) {
		$__compilerTemp2 .= '
			...
			'title' => ($__templater->method($__vars['article'], 'canUseInlineModeration', array()) ? 'Chọn để kiểm duyệt' : ''),
			'_type' => 'option',
		))) . '</li>
						';
	}
	...
	if ($__vars['article']['has_poll']) {
		$__compilerTemp2 .= '
							<li>
								<i class="structItem-status structItem-status--poll" aria-hidden="true" title="' . $__templater->filter('Bình chọn', array(array('for_attr', array()),), true) . '"></i>
	...
	if ($__vars['article']['sticky'] AND $__vars['showSticky']) {
		$__compilerTemp2 .= '
							<li>
								<i class="structItem-status structItem-status--sticky" aria-hidden="true" title="' . $__templater->filter('Ghim lại', array(array('for_attr', array()),), true) . '"></i>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['article']['article_state'] == 'draft') {
		$__compilerTemp1 .= '
							<li>
								<i class="structItem-status structItem-status--moderated" aria-hidden="true" title="' . $__templater->filter('Bị xóa', array(array('for_attr', array()),), true) . '"></i>
								<span class="u-srOnly">' . 'draft' . '</span>
							</li>
						';
	}
	...
	if ($__vars['article']['reaction_score'] AND $__vars['extras']['reaction_score']) {
		$__finalCompiled .= '
						<li>
							<span class="u-muted">' . 'Điểm tương tác' . ': 
		...
		if ($__vars['article']['comment_count'] AND $__vars['extras']['comment_count']) {
			$__finalCompiled .= '
							<dl class="pairs pairs--justified structItem-minor structItem-metaItem structItem-metaItem--comments">
								<dt><a href="' . $__templater->func('link', array('ams', $__vars['article'], ), true) . '#comments" class="u-concealed">' . 'Bình luận' . '</a></dt>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['article']['article_state'] == 'moderated') {
		$__finalCompiled .= '
			...
			'title' => $__templater->filter('Chờ phê duyệt', array(array('for_attr', array()),), false),
		)) . '</li>
						';
	}
	$__finalCompiled .= '
	...
	if ($__vars['article']['article_state'] != 'deleted') {
		...
		if ($__vars['article']['last_update'] > $__vars['article']['publish_date']) {
			$__finalCompiled .= '								
								<li>
									<span class="u-muted">' . 'Cập nhật' . '</span>
									' . $__templater->func('date_dynamic', array($__vars['article']['last_update'], array(
			))) . '
								</li>
							';
		...
		if ($__vars['article']['view_count'] AND $__vars['extras']['view_count']) {
			$__finalCompiled .= '
								<li>
									<span class="u-muted">' . 'Lượt xem' . ': </span>
		...
		if ($__vars['article']['reaction_score'] AND $__vars['extras']['reaction_score']) {
			$__finalCompiled .= '
								<li>
									<span class="u-muted">' . 'Điểm tương tác' . ': </span>
		...
		if ($__vars['article']['Category']['allow_comments'] AND ($__vars['article']['comment_count'] AND $__vars['extras']['comment_count'])) {
			$__finalCompiled .= '
								<li>
									<a href="' . $__templater->func('link', array('ams', $__vars['article'], ), true) . '#comments" class="u-muted"><span>' . 'Bình luận' . ':</span> ' . $__templater->filter($__vars['article']['comment_count'], array(array('number_short', array()),), true) . '</a>
								</li>
							';
		}
		$__finalCompiled .= '
	...
	if ($__vars['extras']['view_article_button'] OR (($__vars['xf']['options']['enableTagging'] AND ($__vars['article']['tags'] AND $__vars['extras']['tags'])))) {
		...
		if ($__vars['xf']['options']['enableTagging'] AND ($__vars['article']['tags'] AND $__vars['extras']['tags'])) {
			$__compilerTemp3 .= '
				...
				'title' => $__templater->filter('Từ khóa', array(array('for_attr', array()),), false),
			)) . '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['item']['CoverImage']) {
		...
		$__finalCompiled .= '
			...
			'defaultname' => ($__vars['item']['username'] ?: 'Thành viên đã bị xoá'),
		))) . '
				';
	}
	$__finalCompiled .= '
	...
	if ($__vars['item']['category_id'] AND $__vars['item']['Category']) {
		$__finalCompiled .= '
						<li>' . 'Chuyên mục' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->escape($__vars['item']['Category']['title']) . '</li>
					';
	}
	$__finalCompiled .= '
				</ul>
