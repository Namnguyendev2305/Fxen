<?php
// FROM HASH: 53171f1dcb160868cd72c6dc08d02ceb
return array(
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped($__templater->func('prefix', array('ams_article', $__vars['article'], 'escaped', ), true) . ($__vars['article']['meta_title'] ? $__templater->escape($__vars['article']['meta_title']) : $__templater->escape($__vars['article']['title'])) . ' - ' . 'Reviews');
	$__templater->pageParams['pageNumber'] = $__vars['page'];
	$__finalCompiled .= '

' . $__templater->callMacro('metadata_macros', 'canonical_url', array(
		'canonicalUrl' => $__templater->func('link', array('canonical:ams/reviews', $__vars['article'], array('page' => (($__vars['page'] > 1) ? $__vars['page'] : null), ), ), false),
	), $__vars) . '
...
' . $__templater->callMacro('lightbox_macros', 'setup', array(
		'canViewAttachments' => $__templater->method($__vars['article'], 'canViewReviewAttachments', array()),
	), $__vars) . '
...
';
	$__vars['sortOrders'] = array('rating_date' => 'Ngày', 'vote_score' => 'Most helpful', 'rating' => 'Bình chọn', );
	$__finalCompiled .= '
...
<div class="block block--messages"
		...
		<div class="block-tabHeader tabs">
			<div class="hScroller" data-xf-init="h-scroller">
				<span class="hScroller-scroll">
					' . '
					<a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__vars['reviewTabs']['latest']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['latest']['selected'] ? 'is-active' : '') . '">' . 'Mới nhất' . '</a>
					';
	if ($__vars['article']['Category']['review_voting']) {
		$__finalCompiled .= '
						<a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__vars['reviewTabs']['helpful']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['helpful']['selected'] ? 'is-active' : '') . '">' . 'Most helpful' . '</a>
					';
	}
	$__finalCompiled .= '
					<a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__vars['reviewTabs']['rating']['filters'], ), true) . '" class="tabs-tab ' . ($__vars['reviewTabs']['rating']['selected'] ? 'is-active' : '') . '">' . 'Bình chọn' . '</a>
					' . '
				</span>
			</div>
			...
			<div class="tabs-extra tabs-extra--minor">
				...
				<div class="menu menu--wide" data-menu="menu" aria-hidden="true"
					data-href="' . $__templater->func('link', array('ams/reviews/filters', $__vars['article'], $__vars['filters'], ), true) . '"
	...
	if ($__vars['filters']['rating']) {
		$__compilerTemp3 .= '
							<li><a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__templater->filter($__vars['filters'], array(array('replace', array(array('rating' => null, ), )),), false), ), true) . '"
	...
	if ($__vars['filters']['term']) {
		$__compilerTemp3 .= '
							<li><a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__templater->filter($__vars['filters'], array(array('replace', array('term', null, )),), false), ), true) . '"
								class="filterBar-filterToggle" data-xf-init="tooltip" title="' . $__templater->filter('Xóa bộ lọc này', array(array('for_attr', array()),), true) . '">
	...
	if ($__vars['filters']['order'] AND $__vars['sortOrders'][$__vars['filters']['order']]) {
		...
		if ((!$__vars['reviewTabs']['latest']['selected']) AND ((!$__vars['reviewTabs']['helpful']['selected']) AND (!$__vars['reviewTabs']['rating']['selected']))) {
			$__compilerTemp3 .= '
								<li><a href="' . $__templater->func('link', array('ams/reviews', $__vars['article'], $__templater->filter($__vars['filters'], array(array('replace', array(array('order' => null, 'direction' => null, ), )),), false), ), true) . '"
