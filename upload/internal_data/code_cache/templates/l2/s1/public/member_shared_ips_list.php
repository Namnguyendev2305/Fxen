<?php
// FROM HASH: 467674ac5739ba8ebf90248875f7385e
return array(
'macros' => array('shared_block' => array(
	...
	if ($__vars['shared']) {
				...
				$__finalCompiled .= '
					...
					'notooltip' => 'true',
								...
								<div class="contentRow-main">
									';
				if ($__templater->method($__vars['xf']['visitor'], 'canCleanSpam', array()) AND $__templater->method($__vars['share']['user'], 'isPossibleSpammer', array())) {
					$__finalCompiled .= '
		...
		$__finalCompiled .= '
				...
				<div class="block-footer">
					<span class="block-footer-counter">' . '' . $__templater->func('count', array($__vars['shared'], ), true) . ' thành viên phù hợp.' . '</span>
				</div>
			';
	} else {
		$__finalCompiled .= '
				<div class="block-body block-row">' . 'Không tìm thấy thành viên phù hợp.' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Thành viên cùng IP với ' . $__templater->escape($__vars['user']['username']) . '');
	$__finalCompiled .= '
