<?php
// FROM HASH: a14d7584e5adb9ce637a0b1967a18d51
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
		...
		if ($__templater->isTraversable($__vars['poll']['responses'])) {
			foreach ($__vars['poll']['responses'] AS $__vars['responseId'] => $__vars['response']) {
				$__compilerTemp1[] = array(
					...
					'selected' => $__templater->method($__vars['poll'], 'hasVoted', array($__vars['responseId'], )),
	...
	if ($__vars['poll']['max_votes'] != 1) {
		$__compilerTemp3 .= '
						<li>' . 'Được phép nhiều bình chọn' . '</li>
					';
	}
	$__compilerTemp3 .= '
					';
	...
	<div class="block-footer">
		<span class="block-footer-controls">
			' . $__templater->button('Bình chọn', array(
		'type' => 'submit',
	...
	if ($__templater->method($__vars['poll'], 'canViewResults', array())) {
		$__finalCompiled .= '
				' . $__templater->button('Xem kết quả', array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
				...
				if ($__vars['canShowVoters']) {
					$__finalCompiled .= '
									<a class="fauxBlockLink-blockLink"
										...
										role="button"
										tabindex="0">
										<span class="u-muted">' . 'Bình chọn' . $__vars['xf']['language']['label_separator'] . '</span> ' . $__templater->filter($__vars['response']['response_vote_count'], array(array('number', array()),), true) . '
									</a>
								';
				} else {
					$__finalCompiled .= '
	...
	if (!$__vars['canViewResults']) {
		$__finalCompiled .= '
			<hr class="block-separator" />
			<div class="block-row block-row--minor">
				<span class="u-muted">' . 'Kết quả chỉ có thể xem được sau khi bình chọn.' . '</span>
			</div>
		';
	...
	if ($__vars['showFooter']) {
		...
		if ($__vars['simpleDisplay']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->escape($__templater->method($__vars['poll'], 'getLink', array('content', ))) . '">' . 'Xem bình luận' . $__vars['xf']['language']['ellipsis'] . '</a>
				';
		} else {
			$__compilerTemp1 .= '
					';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
	$__vars['responsesId'] = $__templater->func('unique_id', array(), false);
	$__finalCompiled .= '
	';
	$__compilerTemp1 = '';
	if ($__vars['poll']) {
			...
			foreach ($__vars['poll']['Responses'] AS $__vars['response']) {
				...
				if ($__templater->method($__vars['poll'], 'canEditDetails', array())) {
					$__compilerTemp1 .= '
						...
						'aria-label' => $__templater->filter('Lựa chọn', array(array('for_attr', array()),), false),
						'placeholder' => 'Lựa chọn' . $__vars['xf']['language']['ellipsis'],
					)) . '
						';
				} else {
					$__compilerTemp1 .= '
	...
	$__finalCompiled .= $__templater->formRow('
		...
		<ul class="inputList" role="group" aria-labelledby="' . $__templater->escape($__vars['responsesId']) . '">
			' . $__compilerTemp1 . '
			' . $__compilerTemp2 . '
		</ul>
	', array(
		'rowtype' => 'input ' . $__vars['rowType'],
		'label' => 'Các đáp án để bình chọn',
		'labelid' => $__vars['responsesId'],
	)) . '
	...
	$__finalCompiled .= $__templater->formRadioRow(array(
		...
		'label' => 'Trả lời có thể lựa chọn tối đa',
		'explain' => 'Đây là số lượng phản hồi tối đa mà thành viên có thể lựa chọn khi bỏ phiếu',
		'rowtype' => $__vars['rowType'],
	)) . '
	...
	$__compilerTemp4 = array(array(
		...
		'label' => '
			' . 'Cho phép thành viên thay đổi phiếu bầu của họ' . '
		',
		'_type' => 'option',
	));
	...
	$__compilerTemp4[] = array(
		...
		'label' => '
			' . 'Cho phép xem các kết quả mà không cần bình chọn' . '
		',
		'_type' => 'option',
	);
	if ($__vars['poll'] AND $__vars['poll']['close_date']) {
		$__compilerTemp4[] = array(
			...
			'label' => '
				' . 'Đóng bình chọn này vào ' . $__templater->func('date_time', array($__vars['poll']['close_date'], ), true) . '' . '
			',
			'_type' => 'option',
		);
	} else {
		$__compilerTemp4[] = array(
			...
			'label' => 'Đóng bình chọn này sau' . $__vars['xf']['language']['label_separator'],
			'_dependent' => array('
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['poll']['voter_count']) {
		$__compilerTemp1[] = array(
			'value' => 'reset',
			'label' => 'Đặt lại toàn bộ bình chọn',
			'_type' => 'option',
		);
	}
	$__compilerTemp1[] = array(
		'value' => 'remove',
		'label' => 'Xóa toàn bộ cuộc bình chọn',
		'_type' => 'option',
	);
	$__finalCompiled .= $__templater->form('
		<div class="block-container">
			<div class="block-body">
				<h2 class="block-header">' . 'Xóa bình chọn' . '</h2>
		...
		'name' => 'poll_action',
			...
			' . $__templater->func('redirect_input', array(null, $__templater->method($__vars['poll'], 'getLink', array('content', )), true)) . '
