<?php
// FROM HASH: 6a024cd6fba65e71532bbf7df04e7a86
return array(
...
<div class="block">
	...
	if ($__vars['user']['Profile']['birthday']['timeStamp']) {
		...
		if ($__vars['user']['Profile']['birthday']['age']) {
			$__compilerTemp3 .= '
											' . $__templater->func('parens', array(('Tuổi' . $__vars['xf']['language']['label_separator'] . ' ') . $__vars['user']['Profile']['birthday']['age'], ), true) . '
										';
		}
		$__compilerTemp3 .= '
									';
	...
	if ($__vars['user']['Profile']['website']) {
		$__compilerTemp2 .= '
						<dl class="pairs pairs--columns pairs--fixedSmall">
							<dt>' . 'Website' . '</dt>
							<dd>
								<a href="' . $__templater->escape($__vars['user']['Profile']['website']) . '" rel="nofollow" target="_blank">' . $__templater->filter($__vars['user']['Profile']['website'], array(array('url_display', array()),), true) . '</a>
							</dd>
						</dl>
					';
	}
	...
	if ($__templater->method($__vars['user'], 'canViewIdentities', array())) {
		...
		if ($__templater->method($__vars['xf']['visitor'], 'canStartConversationWith', array($__vars['user'], ))) {
			$__compilerTemp4 .= '
								<dl class="pairs pairs--columns pairs--fixedSmall">
									<dt>' . 'Trò chuyện' . '</dt>
									<dd><a href="' . $__templater->func('link', array('conversations/add', '', array('to' => $__vars['user']['username'], ), ), true) . '">' . 'Tạo cuộc trò chuyện' . '</a></dd>
								</dl>
							';
		}
		$__compilerTemp4 .= '
	...
	if ($__vars['followingCount']) {
		...
		if (strlen(trim($__compilerTemp5)) > 0) {
			...
			if ($__vars['followingCount'] > $__templater->func('count', array($__vars['following'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/following', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '... và ' . ($__vars['followingCount'] - $__templater->func('count', array($__vars['following'], ), false)) . ' nữa.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
	...
	if ($__vars['followersCount']) {
		...
		if (strlen(trim($__compilerTemp6)) > 0) {
			...
			if ($__vars['followersCount'] > $__templater->func('count', array($__vars['followers'], ), false)) {
				$__compilerTemp1 .= '
							<a href="' . $__templater->func('link', array('members/followers', $__vars['user'], ), true) . '" data-xf-click="overlay">' . '... và ' . ($__vars['followersCount'] - $__templater->func('count', array($__vars['followers'], ), false)) . ' nữa.' . '</a>
						';
			}
			$__compilerTemp1 .= '
					</div>
