...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	<div class="menu menu--pageJump" data-menu="menu" aria-hidden="true">
		...
		'data-menu-autofocus' => 'true',
					...
					<span class="inputGroup-text">' . $__templater->button('Tới', array(
		'class' => 'js-pageJumpGo',
	), '', array(
	)) . '</span>
				</div>
...
<div class="pageNav ' . ($__vars['hasSkipStart'] ? 'pageNav--skipStart' : '') . ' ' . ($__vars['hasSkipEnd'] ? 'pageNav--skipEnd' : '') . '">
	...
	if ($__vars['prev']) {
		$__finalCompiled .= '
		<a href="' . $__templater->escape($__vars['prev']) . '" class="pageNav-jump pageNav-jump--prev">' . 'Trước' . '</a>
	';
	}
	$__finalCompiled .= '
		...
		if ($__vars['startInner'] == 3) {
			...
			$__finalCompiled .= '
				<li class="pageNav-page pageNav-page--skip pageNav-page--skipStart">
					<a data-xf-init="tooltip" title="' . $__templater->filter('Đi đến trang', array(array('for_attr', array()),), true) . '"
						data-xf-click="menu"
	...
	if ($__vars['next']) {
		$__finalCompiled .= '
		<a href="' . $__templater->escape($__vars['next']) . '" class="pageNav-jump pageNav-jump--next">' . 'Tiếp' . '</a>
	';
	}
	$__finalCompiled .= '
</div>
...
<div class="pageNavSimple">
	...
	<a class="pageNavSimple-el pageNavSimple-el--current"
		data-xf-init="tooltip" title="' . $__templater->filter('Đi đến trang', array(array('for_attr', array()),), true) . '"
