...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Nâng cấp tài khoản');
	$__finalCompiled .= '
...
';
		...
		$__compilerTemp1 .= '
			<div class="block">
				<div class="block-container">
					<h2 class="block-header">' . 'Các gói nâng cấp hiện tại' . '</h2>
			...
			foreach ($__vars['available'] AS $__vars['upgrade']) {
				...
				if (($__templater->func('count', array($__vars['upgrade']['payment_profile_ids'], ), false) > 1)) {
					...
					$__compilerTemp3 = array(array(
						'label' => $__vars['xf']['language']['parenthesis_open'] . 'Chọn phương thức thanh toán' . $__vars['xf']['language']['parenthesis_close'],
						'_type' => 'option',
					));
	...
	if (!$__templater->test($__vars['purchased'], 'empty', array())) {
		$__compilerTemp1 .= '
			<div class="block">
				<div class="block-container">
					<h2 class="block-header">' . 'Nâng cấp đã mua' . '</h2>
		...
		if ($__templater->isTraversable($__vars['purchased'])) {
			foreach ($__vars['purchased'] AS $__vars['upgrade']) {
				...
				if ($__vars['active']['end_date']) {
					$__compilerTemp4 .= '
										' . 'Hết hạn' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['active']['end_date'], array(
					))) . '
									';
				} else {
					$__compilerTemp4 .= '
...
';
		...
		$__finalCompiled .= '
	<div class="blockMessage">' . 'Hiện tại không có nhóm thành viên có thể mua để nâng cấp.' . '</div>
';
	}
	$__finalCompiled .= '
';
