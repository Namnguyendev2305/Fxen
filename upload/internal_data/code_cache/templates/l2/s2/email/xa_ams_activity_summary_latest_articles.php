	...
	';
		...
		';
				...
				if ($__vars['article']['last_update'] > $__vars['article']['publish_date']) {
					$__compilerTemp1 .= '								
				&middot; <span class="u-muted">' . 'Cập nhật' . '</span>
			...
			';
				...
				if ($__vars['article']['view_count']) {
					$__compilerTemp4 .= '
				&middot; ' . 'Lượt xem' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['article']['view_count'], array(array('number_short', array()),), true) . '
			';
				}
				$__compilerTemp5 = '';
				if ($__vars['article']['Category']['allow_comments'] AND $__vars['article']['comment_count']) {
					$__compilerTemp5 .= '
				&middot; ' . 'Bình luận' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['article']['comment_count'], array(array('number_short', array()),), true) . '
			';
				}
				$__compilerTemp6 = '';
			...
			';
				...
				if ($__vars['article']['reaction_score']) {
					$__compilerTemp7 .= '
				&middot; ' . 'Điểm tương tác' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->filter($__vars['article']['reaction_score'], array(array('number_short', array()),), true) . '
			';
				}
		...
		';
			...
			' . 'Chuyên mục' . $__vars['xf']['language']['label_separator'] . ' <a href="' . $__templater->func('link', array('canonical:ams/categories', $__vars['article']['Category'], ), true) . '">' . $__templater->escape($__vars['article']['Category']['title']) . '</a>
		');
				$__finalCompiled .= '
