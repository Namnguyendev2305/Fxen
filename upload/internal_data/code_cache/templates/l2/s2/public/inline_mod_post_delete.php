...
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Quản lý trực tuyến - Xóa các bài viết');
	$__finalCompiled .= '
...
';
	...
	if ($__vars['firstPostCount']) {
		$__compilerTemp1 .= '
				' . $__templater->formInfoRow('
					<div class="blockMessage blockMessage--important"><strong>' . 'Chú ý' . $__vars['xf']['language']['label_separator'] . '</strong> ' . '' . $__templater->filter($__vars['firstPostCount'], array(array('number', array()),), true) . ' thread(s) will be deleted when these posts are deleted.' . '</div>
				', array(
		)) . '
			';
	}
	...
	<div class="block-container">
		<div class="block-body">
			' . $__templater->formInfoRow('Bạn có chắc muốn xóa ' . $__templater->escape($__vars['total']) . ' bài viết?', array(
		'rowtype' => 'confirm',
	)) . '
