<?php
// FROM HASH: aa5f0f0412cb2a56a0dfe10e09df9a9c
return array(
...
'user_cell' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
		...
		'includeMicrodata' => $__vars['includeMicrodata'],
		'itemProp' => 'author',
	), $__vars) . '
					</div>
				';
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	';
	$__vars['includeMicrodata'] = (!$__templater->method($__vars['post'], 'isFirstPost', array()));
	$__finalCompiled .= '
	';
	$__templater->includeCss('message.less');
	$__finalCompiled .= '
';
	if ($__vars['post']['User']['is_staff']) {
		$__finalCompiled .= '
	';
		$__templater->includeCss('th_uix_staffPost.less');
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
';
	if ($__vars['post']['Thread']['user_id'] === $__vars['post']['user_id']) {
		$__finalCompiled .= '
	';
		$__templater->includeCss('th_uix_threadStarterPost.less');
		$__finalCompiled .= '
';
	}
	$__finalCompiled .= '
	...
	' . '
	<article class="message  ' . ($__vars['post']['User']['is_staff'] ? ' message-staffPost ' : '') . ' ' . (($__vars['post']['Thread']['user_id'] === $__vars['post']['user_id']) ? ' message-threadStarterPost ' : '') . $__templater->escape($__templater->renderExtension('extra_classes', $__vars, $__extensions)) . ' js-post js-inlineModContainer ' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? 'is-ignored' : '') . ' ' . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . '"
		...
		';
	if ($__vars['includeMicrodata']) {
		$__finalCompiled .= '
			<meta itemprop="mainEntity" itemscope itemtype="https://schema.org/DiscussionForumPosting" itemid="' . $__templater->func('link', array('canonical:threads', $__vars['thread'], ), true) . '" />
		';
	}
	$__finalCompiled .= '
...
'arguments' => function($__templater, array $__vars) { return array(
		...
		'repliesPhrase' => 'Trả lời' . $__vars['xf']['language']['label_separator'],
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['showThreadStateIcons']) {
		...
		if ($__vars['thread']['sticky']) {
			$__finalCompiled .= '
				...
				'title' => $__templater->filter('Ghim lại', array(array('for_attr', array()),), false),
			)) . '
	...
	if ($__vars['showUser']) {
		...
		$__finalCompiled .= '
				<li class="u-concealed message-attribution-user--top">' . 'Bởi ' . $__templater->escape($__vars['username']) . '' . '</li>
			';
	}
	$__finalCompiled .= '
	...
	if ($__templater->method($__vars['post'], 'isUnread', array())) {
		$__finalCompiled .= '
				<li><span class="message-newIndicator">' . 'Mới' . '</span></li>
			';
	} else if ($__templater->method($__vars['thread'], 'isUnread', array()) AND $__vars['showThreadUnreadIndicator']) {
		$__finalCompiled .= '
				<li><span class="message-newIndicator" title="' . $__templater->filter('New replies', array(array('for_attr', array()),), true) . '">' . 'Mới' . '</span></li>
			';
	}
	$__finalCompiled .= '
			<li>
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
			...
			<div ' . ($__vars['includeMicrodata'] ? 'itemprop="text"' : '') . '>
				';
	if ($__vars['snippet']) {
		$__finalCompiled .= '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
				' . 'Sửa lần cuối' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
			';
		} else {
			$__finalCompiled .= '
				' . 'Sửa bởi Amin' . $__vars['xf']['language']['label_separator'] . ' ' . $__templater->func('date_dynamic', array($__vars['post']['last_edit_date'], array(
				'itemprop' => 'dateModified',
			))) . '
			';
		}
		...
		if ($__vars['xf']['options']['multiQuote'] AND $__vars['xf']['visitor']['user_id']) {
			$__compilerTemp1 .= '
					<a href="' . $__templater->escape($__vars['quoteLink']) . '"
						...
						data-mq-action="add">' . 'Trích dẫn' . '</a>
				';
		}
		$__compilerTemp1 .= '
				...
				<a href="' . $__templater->escape($__vars['quoteLink']) . '"
					...
					rel="nofollow"
					data-xf-click="quote"
					data-quote-href="' . $__templater->func('link', array('posts/quote', $__vars['post'], ), true) . '">' . 'Trả lời' . '</a>
			';
	}
	$__compilerTemp1 .= '
		';
	...
	if (($__vars['post']['message_state'] == 'deleted') AND $__templater->method($__vars['post'], 'canUndelete', array())) {
		$__compilerTemp2 .= '
				<a href="' . $__templater->func('link', array('posts/undelete', $__vars['post'], ), true) . '"
					...
					data-xf-click="overlay">' . 'Khôi phục' . '</a>
...
';
	...
	<div class="message message--deleted message--post' . ($__templater->method($__vars['post'], 'isIgnored', array()) ? ' is-ignored' : '') . ($__templater->method($__vars['post'], 'isUnread', array()) ? ' is-unread' : '') . ' js-post js-inlineModContainer"
		...
		'log' => $__vars['post']['DeletionLog'],
					...
					<a href="' . $__templater->func('link', array('posts/show', $__vars['post'], ), true) . '" class="u-jsOnly" data-xf-click="inserter" data-replace="[data-content=post-' . $__templater->escape($__vars['post']['post_id']) . ']">' . 'Hiển thị' . $__vars['xf']['language']['ellipsis'] . '</a>
