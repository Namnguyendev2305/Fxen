<?php
// FROM HASH: b599619a3599e62aa01b911e00336595
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['user']['user_id'] == $__vars['xf']['visitor']['user_id']) {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Your drafts');
...
';
		...
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Drafts by ' . $__templater->escape($__vars['user']['Profile']['xa_ams_author_name']) . '');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
';
	} else {
		$__finalCompiled .= '
	';
		$__templater->pageParams['pageTitle'] = $__templater->preEscaped('Drafts by ' . $__templater->escape($__vars['user']['username']) . '');
		$__templater->pageParams['pageNumber'] = $__vars['page'];
		$__finalCompiled .= '
';
	}
...
<div class="block" data-type="ams_article">
	...
	<div class="block-container">
		<div class="block-body">
			';
	if (!$__templater->test($__vars['drafts'], 'empty', array())) {
		$__finalCompiled .= '
					<div class="structItemContainer structItemContainerAmsListView">
						';
		if ($__templater->isTraversable($__vars['drafts'])) {
			foreach ($__vars['drafts'] AS $__vars['article']) {
				$__finalCompiled .= '
